...
class ReadingMangaServiceImplTest {
    ...
    void getAllReadingMangasByUserIdShouldReturnReadingMangasWhenPageSizeIsWithinLimit() {
        ...
        when(readingMangaRepository.findByUserId(USER_ID, pageable)).thenReturn(readingMangaEntities);
        when(readingMangaMapper.toDto(any(ReadingMangaEntity.class))).thenReturn(new ReadingMangaDto());

        // When
        Slice<ReadingMangaDto> result = readingMangaService.getAllReadingMangasByUserId(USER_ID, pageable);

        // Then
        assertThat(result).isNotNull();
        assertThat(result.getContent()).isNotEmpty();
        verify(readingMangaRepository).findByUserId(USER_ID, pageable);
